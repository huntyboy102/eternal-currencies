...
public class QuestsIntegrationClient {
    ...
    public static void setupRewardGuiProviders() {
        QuestsIntegration.Rewards.CURRENCY.setGuiProvider((panel, quest, callback) -> {
            ...
            EditStringConfigOverlay<Long> overlay = new EditStringConfigOverlay<>(panel.getGui(), config, accepted -> {
                if(accepted)
                    callback.accept(new CurrencyReward(0L, quest, ResourceLocation.fromNamespaceAndPath(EternalCurrencies.MODID, "coins"), config.getValue()));
            ...
            EnumConfig<ResourceLocation> enumConfig = new EnumConfig<>(NameMap.of(ResourceLocation.fromNamespaceAndPath(EternalCurrencies.MODID, "coins"), currencies)
